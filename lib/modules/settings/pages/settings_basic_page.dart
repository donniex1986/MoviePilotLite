...
class SettingsBasicPage extends GetView<SettingsBasicController> {
    ...
    return Obx(() {
      ...
      return CupertinoPageScaffold(
        navigationBar: CupertinoNavigationBar(
          ...
          middle: const Text('基础', style: TextStyle(fontWeight: FontWeight.w600)),
          trailing: editing
              ? Row(
            ...
            CustomScrollView(
              slivers: [
                ...
                if (controller.isEditing.value
                    ? controller.aiSectionExpanded.value
                    : controller.aiAgentEnabled)
                  _buildSection(
                    context,
                    header: '全局智能助手',
  ...
  Widget _buildFieldRow(BuildContext context, SettingsFieldConfig field) {
    ...
    final editable = controller.isEditing.value && field.type != SettingsFieldType.textCopy;
    ...
    final isAiEnableField = field.envKey == 'AI_AGENT_ENABLE';
    return SettingsFieldRow(
      ...
      onCopyTap: field.type == SettingsFieldType.textCopy
          ...
          : null,
      editable: editable,
      textController: (editable && (isText || isNumber))
          ? controller.textControllerFor(field)
          : null,
      onTextChanged: (editable && isText) ? (s) => _onFieldChanged(field, s) : null,
      onTextSubmitted: (editable && isText)
          ? (s) => _onFieldChanged(field, s)
          : null,
      switchValue: isAiEnableField && controller.isEditing.value
          ? controller.aiSectionExpanded.value
          : _parseBool(value),
      onSwitchChanged: editable && isToggle
          ? (v) {
              if (isAiEnableField) {
                controller.setAiSectionExpanded(v);
              } else {
                _onFieldChanged(field, v);
              }
            }
          : null,
      step: field.step,
      onNumberChanged: editable && isNumber
          ? (n) {
      ...
      onSelectChanged: editable && isSelect && selectOpts != null && selectOpts.isNotEmpty
