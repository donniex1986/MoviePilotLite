...
class SiteResourceItemCard extends StatelessWidget {
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const Spacer(),
              if (item.seeders > 0) ...[
                Icon(Icons.arrow_upward, size: 18, color: Colors.green.shade600),
              ...
              if (item.peers > 0) ...[
                Icon(Icons.arrow_downward, size: 18, color: Colors.red.shade600),
                const SizedBox(width: 4),
                Text(
                  '${item.peers}',
                  style: theme.textTheme.bodySmall?.copyWith(
          ...
          if (item.description.isNotEmpty) const SizedBox(height: 8),
          // hit_and_run 重点显示
          if (item.hitAndRun) ...[
          ...
          // 体积因子 / 促销
          if (item.volumeFactor.isNotEmpty) ...[
  ...
  List<Widget> _buildLabelChips() {
    ...
    return item.labels.take(6).map((label) {
