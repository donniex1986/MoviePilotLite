...
class QBTorrentModel {
  ...
  static String _getLocalizedStateText(String state) {
    ...
    // qBittorrent 状态到本地化字符串的映射
    switch (stateLower) {
      // 基本状态
      case 'downloading':
        return QBLocalizable.downloading;
      case 'seeding':
        return QBLocalizable.seeding;
      case 'stopped':
        return QBLocalizable.stopped;
      case 'uploading':
        return QBLocalizable.uploading;

      // 暂停状态
      case 'pauseddl':
        return QBLocalizable.pausedDownload;
      case 'pausedup':
        return QBLocalizable.pausedUpload;
      case 'paused':
        return QBLocalizable.paused;

      // 强制状态
      case 'forceddl':
      case 'forced_download':
        return QBLocalizable.forcedDownload;
      case 'forcedup':
      case 'forced_upload':
        return QBLocalizable.forcedUpload;
      case 'forcedmetadata':
      case 'forced_meta_download':
        return QBLocalizable.forcedMetaDownload;

      // 元数据下载
      case 'metadl':
      case 'meta_download':
        return QBLocalizable.metaDownload;

      // 停滞状态
      case 'stalleddl':
      case 'stalled_download':
        return QBLocalizable.stalledDownload;
      case 'stalledup':
      case 'stalled_upload':
        return QBLocalizable.stalledUpload;

      // 检查状态
      case 'checking':
        return QBLocalizable.checking;
      case 'checkingdl':
      case 'checking_download':
        return QBLocalizable.checkingDownload;
      case 'checkingup':
      case 'checking_upload':
        return QBLocalizable.checkingUpload;
      case 'checkingresumedata':
      case 'checking_resume_data':
        return QBLocalizable.checkingResumeData;

      // 排队状态
      case 'queueddl':
      case 'queued_download':
        return QBLocalizable.queuedDownload;
      case 'queuedup':
      case 'queued_upload':
        return QBLocalizable.queuedUpload;
      case 'queued_to_seed':
        return QBLocalizable.queuedToSeed;

      // 其他状态
      case 'moving':
        return QBLocalizable.moving;
      case 'error':
        return QBLocalizable.error;
      case 'missingfiles':
      case 'missing_files':
        return QBLocalizable.missingFiles;
      case 'stoppedUP':
        return QBLocalizable.stopped;
      // 未知状态
      default:
        return QBLocalizable.unknown;
    }
  }
